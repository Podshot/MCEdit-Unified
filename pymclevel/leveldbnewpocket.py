...
class PocketLeveldbDatabase(object):
    ...
    def world_db(self):
        ...
        if PocketLeveldbDatabase.holdDatabaseOpen:
            if self._world_db is None:
                self._world_db = leveldb_mcpe.DB(os.path.join(str(self.path), "db"))
            yield self._world_db
            pass
        else:
            db = leveldb_mcpe.DB(os.path.join(str(self.path), "db"))
            yield db
            del db
    ...
    def savePlayer(self, player, playerData, batch=None):
        if batch is None:
            with self.world_db() as db:
                db.Put(player, playerData)
        else:
