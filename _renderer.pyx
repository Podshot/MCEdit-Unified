...
from cpython cimport PyTypeObject, PyList_Append
...
cdef chunkMarkers(chunkSet):
    """ Returns a mapping { size: [position, ...] } for different powers of 2
    as size.
    """

    cdef int size, nextsize, cx, cz
    cdef dict sizedChunks
...
cdef class ChunkRenderer(object):
    #cdef int maxlod, minlod
    maxlod = 2
    minlod = 0

    def __init__(self, renderer, chunkPosition):
        cdef dict renderstateLists
        cdef int detailLevel, bufferSize
        cdef list blockRenderers
        cdef set invalidLayers
    ...
    cdef void makeDisplayLists(self):
        cdef dict lists
        cdef list renderers
        cdef long l
    ...
    cdef void invalidate(self, list layers=None):
        cdef set layer
        cdef list blockRenderers
        if layers is None:
    ...
    cdef void vertexArraysDone(self):
        bufferSize = 0
        for br in self.blockRenderers:
...
cdef numpy.ndarray[numpy.float64_t, ndim=3] makeVertexTemplatesFromJsonModel(fromVertices, toVertices, uv):
    """
...
cdef numpy.ndarray[numpy.float64_t, ndim=3] rotateTemplate(numpy.ndarray[numpy.float64_t, ndim=3] template, int x=0, int y=0):
    """
...
cpdef numpy.ndarray[numpy.float64_t, ndim=3] makeVertexTemplates(xmin=0, ymin=0,zmin=0, xmax=1, ymax=1, zmax=1):
    return numpy.array((
...
cdef int elementByteLength = 24


cdef list createPrecomputedVertices():
    cdef list precomputedVertices
    cdef int dir
    cdef int height = 16
...
cdef list faceVertexTemplates = makeVertexTemplates()
...
cdef class Layer:
    cdef unicode Blocks, Entities, Monsters, Items, TileEntities, TileTicks, TerrainPopulated, ChunkBorder
    cdef tuple AllLayers
    Blocks = u"Blocks"
    Entities = u"Entities"
    Monsters = u"Monsters"
    Items = u"Items"
    TileEntities = u"TileEntities"
    TileTicks = u"TileTicks"
    TerrainPopulated = u"TerrainPopulated"
    ChunkBorder = u"ChunkBorder"
    AllLayers = (Blocks, Entities, Monsters, Items, TileEntities, TileTicks, TerrainPopulated, ChunkBorder)


cdef class BlockRenderer(object):
    cdef tuple detailLevels
    cdef unicode layer
    cdef dict directionOffsets
    cdef object renderstate
    cdef bint used
    detailLevels = (0,)
    layer = Layer.Blocks
    directionOffsets = {
    ...
    def makeChunkVertices(self, chunk):
        if hasattr(chunk, "TileTicks"):
            self.vertexArrays.append(
                self._computeVertices([[tick[j].value for j in "xyz"] for i, tick in enumerate(chunk.TileTicks)],
                                      (0xff, 0xff, 0xff, 0x44),
                                      chunkPosition=chunk.chunkPosition))
        yield
        ...
        for (direction, exposedFaceIndices) in enumerate(facingBlockIndices):
            ...
            if self.materials.name in ("Alpha", "Pocket"):
                if direction == pymclevel.faces.FaceYIncreasing:
                    ...
                    vertexArray.view('uint8')[_RGB][grass] = vertexArray.view('uint8')[_RGB][grass].astype(
                        float) * self.grassColor
            yield
...
class LeafBlockRenderer(BlockRenderer):
    ...
    def makeLeafVertices(self, facingBlockIndices, blocks, blockMaterials, blockData, areaBlockLights, texMap):
        ...
        for (direction, exposedFaceIndices) in enumerate(facingBlockIndices):
            ...
            vertexArray.view('uint8')[_RGB] *= facingBlockLight[blockIndices][..., numpy.newaxis, numpy.newaxis]
            if self.materials.name in ("Alpha", "Pocket"):
                vertexArray.view('uint8')[_RGB][leaves] = vertexArray.view('uint8')[_RGB][leaves].astype(
                    float) * self.leafColor
                vertexArray.view('uint8')[_RGB][pines] = vertexArray.view('uint8')[_RGB][pines].astype(
                    float) * self.pineLeafColor
                vertexArray.view('uint8')[_RGB][birches] = vertexArray.view('uint8')[_RGB][birches].astype(
                    float) * self.birchLeafColor
                vertexArray.view('uint8')[_RGB][jungle] = vertexArray.view('uint8')[_RGB][jungle].astype(
                    float) * self.jungleLeafColor
                vertexArray.view('uint8')[_RGB][acacia] = vertexArray.view('uint8')[_RGB][acacia].astype(
                    float) * self.acaciaLeafColor
                vertexArray.view('uint8')[_RGB][darkoak] = vertexArray.view('uint8')[_RGB][darkoak].astype(
                    float) * self.darkoakLeafColor
    ...
    def makePlantVertices(self, facingBlockIndices, blocks, blockMaterials, blockData, areaBlockLights, texMap):
        ...
        for direction in (
            ...
            if colorize is not None:
                vertexArray.view('uint8')[_RGB][colorize] = vertexArray.view('uint8')[_RGB][colorize].astype(
                    float) * LeafBlockRenderer.leafColor
                vertexArray.view('uint8')[_RGB][colorize2] = vertexArray.view('uint8')[_RGB][colorize2].astype(
                    float) * LeafBlockRenderer.leafColor
            append(vertexArray)
            yield
...
class LeverBlockRenderer(BlockRenderer):
    ...
    leverBaseTemplates = numpy.array([
        ...
        leverBaseTemplate,
        rotateTemplate(leverBaseTemplate, x=0, y=90),
    ...
    leverTemplates = numpy.array([
        ...
        leverTemplate,
        rotateTemplate(leverTemplate, x=0, y=90),
        rotateTemplate(leverTemplate, x=180),
        rotateTemplate(leverTemplate, x=180),
        rotateTemplate(leverTemplate, x=90, y=90),
...
class WallSignBlockRenderer(BlockRenderer):
    ...
    wallSignTemplates = numpy.array([
        wallSignTemplate,
        wallSignTemplate,
        rotateTemplate(wallSignTemplate, x=0, y=180),
        wallSignTemplate,
        rotateTemplate(wallSignTemplate, x=0, y=90),
        rotateTemplate(wallSignTemplate, x=0, y=270),
        numpy.zeros((6, 4, 6)), numpy.zeros((6, 4, 6))
    ])
...
class DoorRenderer(BlockRenderer):
    ...
    doorTemplates = numpy.array([
        # lower hinge left
        doorTemplate,
        rotateTemplate(doorTemplate, x=0, y=90),
        rotateTemplate(doorTemplate, x=0, y=180),
        rotateTemplate(doorTemplate, x=0, y=270),
        rotateTemplate(doorRHTemplate, x=0, y=90),
        rotateTemplate(doorRHTemplate, x=0, y=180),
        rotateTemplate(doorRHTemplate, x=0, y=270),
        doorRHTemplate,
        # upper hinge left
        doorTemplate,
        rotateTemplate(doorTemplate, x=0, y=90),
...
class TrapDoorRenderer(BlockRenderer):
    ...
    trapDoorTemplates = numpy.array([
        ...
        bottomTemplate,
        openTemplate,
        rotateTemplate(openTemplate, x=0, y=180),
        rotateTemplate(openTemplate, x=0, y=270),
        rotateTemplate(openTemplate, x=0, y=90),
        topTemplate,
        topTemplate,
        topTemplate,
        topTemplate,
