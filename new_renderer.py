...
def write_array(fname, array):
    with open('{}.numpy'.format(fname), 'w') as outfile:
        outfile.write('# Array shape: {0}\n'.format(array.shape))
        #outfile.write('# Block: {}\n'.format(theseBlocks[0]))
...
class ChunkCalculator(object):
    ...
    def computeGeometry(self, chunk, areaBlockMats, facingBlockIndices, areaBlockLights, chunkRenderer, blockRenderers):
        global new_renderer
        if chunk.chunkPosition == (0, -1):
            new_renderer = NewBlockRenderer()
            new_renderer.build(chunk)
        #new_block_renderers.append(new_renderer)
...
class CreeperRenderer(BaseEntityRenderer):
    ...
    def makeChunkVertices(self, chunk):
        ...
        for i, ent in enumerate(chunk.Entities):
            if i % 10 == 0:
                yield
            if ent['id'].value != 'minecraft:creeper':
                continue
            pos = pymclevel.Entity.pos(ent)
            pos[1] += 0.5
            creeperPositions.append(pos)
            
        creepers = self._computeVertices(creeperPositions,
                                         (0xff, 0xff, 0xff, 0x44),
                                         offset=True,
                                         chunkPosition=chunk.chunkPosition)
        yield
        self.vertexArrays = [creepers]
    ...
    def makeGenericVertices(self, facingBlockIndices, blocks, blockMaterials, blockData, areaBlockLights, texMap):
        ...
        for (direction, exposedFaceIndices) in enumerate(facingBlockIndices):
            ...
            yield
                ...
                #numpy.savetxt('test.numpy', vertexArray)
...
class LeafBlockRenderer(BlockRenderer):
    ...
    def makeLeafVertices(self, facingBlockIndices, blocks, blockMaterials, blockData, areaBlockLights, texMap):
        ...
        self.vertexArrays = arrays
        write_array('leaves_3', self.vertexArrays[0])
